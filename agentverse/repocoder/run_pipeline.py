...
def run_RG1_and_oracle_method(benchmark, repos, window_sizes, slice_sizes):
    ...
    if not os.path.exists('prompts'):
        os.makedirs('prompts')
    output_file_path = f'prompts/{benchmark}-line1-rg-one-gram-ws-20-ss-2.jsonl'
    ...
    mode = CONSTANTS.gt
    output_file_path = f'prompts/{benchmark}-line1-gt-one-gram-ws-20-ss-2.jsonl'
    BuildPromptWrapper('one-gram', benchmark, repos, window_sizes[0], slice_sizes[0], tokenizer).build_first_search_prompt(mode, output_file_path)
...
if __name__ == '__main__':
    ...
    run_RG1_and_oracle_method(CONSTANTS.sim1_benchmark, repos, window_sizes, slice_sizes)

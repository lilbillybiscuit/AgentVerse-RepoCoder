...
from agentverse.environments.coderagent.basic import BasicEnvironment
...
class CoderAgent:
    ...
    def run(self, group_input):
        """Run the environment from scratch until it is done."""
        self.environment.reset()
        self.logs = []
        n = len(group_input['prompt_group'])
        completion_result = [f'<MASK {i}>' for i in range(n)]
        while not self.environment.is_done():
            completion_result, logs = asyncio.run(
                self.environment.step(group_input, completion_result)
            )
            self.logs += logs
        return completion_result, self.logs
